...
class PNGCanvas:
    ...
    def block(self, x, y, color=None):
        ...
        ey, ex = int((y + 1)*self.width/self.size), int((x + 1) * self.height/self.size)
...
def IDicon(width=73, height=73):
    code = str(random.randint(1000000000, 9999999999))
    col = hex(hash(code))
    color = bytearray(
        [int(col[-9:-7], 16), int(col[-13:-11], 16), int(col[-11:-9], 16)])
    bgcolor = bytearray([0xff-int(col[-7:-5], 16), 0xff - int(col[-3:-1], 16), 0xff-int(col[-5:-3], 16)])
